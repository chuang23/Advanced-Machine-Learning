...
imdb_dir <- "C:/Users/Anniesabol/Desktop/AdvanceML"
...
```{r}
history <- model %>%
  fit(train_data,train_label,epochs=10,batch_size=32,validation_data = list(valid_data,valid_label))
...
glove_dir = "C:/Users/Anniesabol/Desktop/glove.6B"
...
for (word in names(word_index)) {index <- word_index[[word]]
if (index < max_words) {embedding_vector <- embeddings_index[[word]]
if (!is.null(embedding_vector))embedding_matrix[index+1,] <- embedding_vector}}
...
history1 <- model %>% fit(train_data, train_label,epochs = 20,batch_size = 32,validation_data = list(valid_data , valid_label))
plot(history1)
...
model %>% fit(train_data, train_label,epochs = 2,batch_size = 32)
